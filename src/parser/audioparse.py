...
def write_header(header_filename, headername):
    with open(header_filename, 'w') as f:
        macro = headername.upper() + "_AUDIO_H"
        ...
        f.write(f"extern const unsigned char * const {headername}_audio_data;\n")
        f.write(f"extern const unsigned int {headername}_audio_size;\n\n")
...
def write_source(data, source_filename, headername, lastput):
    with open(source_filename, 'w') as f:
        ...
        f.write(f"const unsigned char {headername}_audio_data_array[] = {{\n")
        for i, byte in enumerate(data):
            f.write(f"0x{byte:02x}, ")
            if (i + 1) % 12 == 0:
                f.write("\n")
        f.write("\n};\n\n")
        f.write(f"const unsigned char * const {headername}_audio_data = {headername}_audio_data_array;\n")
        f.write(f"const unsigned int {headername}_audio_size = {len(data)};\n")

def main():
    if len(sys.argv) != 4:
        print(f"Usage: {sys.argv[0]} <input.ogg> <output_basename> <headername>")
        sys.exit(1)
